...
const createQuery = (text: string) => {
  text = text.split(" ")
  const n = 5
  const search = text.map((word, wordIndex) => {
    if(wordIndex + n <= text.length) return text.slice(wordIndex, wordIndex + n).join(' ')
  })
  
  return search.filter(search => search).map(search => `"${search}"`).join(' OR ')
}
