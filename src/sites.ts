...
const makeQueryFunc = (
  ...
  return (node: HTMLElement): string => {
    for (const sel of selector) {
      ...
      if (el) {
        return extractQuery(el as HTMLElement, quoted, startSlice, endSlice, removeQuotes)
      }
    }
    return ''
  }
...
const sites: Sites = {
  ...
  'www.saarbruecker-zeitung.de': {
    ...
    start: () => {
      const fadingEffectClass = 'article-content--reduced'
      ;[...document.getElementsByClassName(fadingEffectClass)].forEach(
        (element) => {
          element.classList.remove(fadingEffectClass)
        },
      )
    },
  },
