...
export default {
  run_update: (currentVersion, previousVersion) => {
    if (previousVersion <= '0.11.0') {
      ...
      console.log(
        'Update from < 0.12.0 - Changing from user options to provider dependent user options',
      )
      ...
      browser.storage.sync.get(defaults).then(
        (items) => {
          if (items.username && items.password) {
            ...
            newStorage.providerOptions['voebb.de.options.username'] =
              items.username
            newStorage.providerOptions['voebb.de.options.password'] =
              items.password
            browser.storage.sync
              .set(newStorage)
              .catch((e) =>
                console.log('Failes to save new entries to storage:', e),
              )
            browser.storage.sync
            ...
            console.log(
              "There isn't any username or any password set, skipping",
            )
          }
        },
